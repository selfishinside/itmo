 ...
 // меняет доступность комнаты
    ...
    private HotelDTO convertToDTO(Hotel hotel) {
        return HotelDTO.builder()
                ...
                .averageReviewRating(hotel.getAverageReviewRating())
    ...
    /** возвращает идентификаторы отелей не раскрывая reviewservice репозиторий отелей */
    @Override
    @Transactional(readOnly = true)
    public List<Long> getAllHotelIds() {
        return hotelRepository.findAll().stream().map(Hotel::getId).toList();
    }

    /** сохраняет рассчитанный reviewservice агрегированный рейтинг отеля */
    @Override
    @Transactional
    public void updateReviewRating(Long hotelId, BigDecimal averageRating, int totalReviews) {
        Hotel hotel = hotelRepository.findById(hotelId)
                .orElseThrow(() -> new IllegalArgumentException("Hotel not found"));
        hotel.setAverageReviewRating(averageRating);
