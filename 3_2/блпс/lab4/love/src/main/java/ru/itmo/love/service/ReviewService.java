...
public class ReviewService implements ReviewServiceInt {
    ...
    public Review moderate(ReviewSubmittedEvent event) {
        ...
        Review saved;
        if (review.getComment() != null && review.getComment().toLowerCase().contains("spam")) {
            saved = reject(review.getId());
        } else {
            saved = publish(review.getId());
        }
        recalculateHotelRating(review.getHotelId());
        return saved;
    }

    /** находит отзыв для шага процесса */
    @Override
    ...
    /** отклоняет отзыв по решению процесса */
    @Override
    @Transactional
    ...
    /** публикует отзыв через коннектор битрикса */
    @Override
    @Transactional
    public Review publish(Long id) {
        ...
        if (!result.success()) throw new IllegalStateException("Bitrix rejected review: " + result.response());
