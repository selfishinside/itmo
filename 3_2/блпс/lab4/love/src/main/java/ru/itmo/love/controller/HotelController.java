 ...
 */
    @GetMapping
    @PreAuthorize("hasAuthority(T(ru.itmo.love.security.policy.Privilege).HOTEL_READ)")
 ...
 */
    @PostMapping
    @PreAuthorize("hasAuthority(T(ru.itmo.love.security.policy.Privilege).HOTEL_CREATE)")
    public ResponseEntity<ApiResponse<HotelDTO>> createHotel(@RequestBody HotelDTO hotelDTO) {
        ...
        try {
            HotelDTO created = hotelService.getHotelById(workflow.create("registerHotel", hotelDTO));
