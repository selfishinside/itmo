            ...
            switch (parts[0]) {
                case "add":
                    try {
                        ...
                        Server.sendRequest(new Request("add", new Vehicle(vehicleData), null));
                ...
                case "updateId":
                    try {
                        ...
                        Server.sendRequest(new Request("updateId " + id, new Vehicle(vehicleData), strId));
                        vehicleData = new String[7];
                        isSpecialCommandExecuted = true;
                ...
                case "addIfMax":
                    ...
                    Server.sendRequest(new Request("addIfMax", new Vehicle(vehicleData), null));
                    vehicleData = new String[7];
                    isSpecialCommandExecuted = true;
                    } catch (NumberFormatException e) {
                        System.out.println("ID must be a valid number: " + e.getMessage());
                ...
                try {
                    if (line.contains("removeById") || line.contains("removeLower") || line.contains("countByFuelType") || line.contains("countLessThenFuelType")) {
                        if (parts.length < 2) {
                            ...
                            Server.sendRequest(new Request(line, null, parts[1]));
                        }
                    } else {
                        Server.sendRequest(new Request(line, null, null));
                    }
                }catch (NoArgumentException e) {
                    System.out.println(e.getMessage());
                    System.out.println("Script " + path + " executing finished with error");
