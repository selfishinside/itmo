...
/**
 * ручки для показа лр4
 */
@RestController
@RequestMapping({"/api/v1/lab4", "/api/v1/lab3"})
@RequiredArgsConstructor
public class Lab3Controller {
    private final ru.itmo.love.bpm.CommandWorkflow workflow;
    ...
    public ResponseEntity<ApiResponse<Integer>> checkInReminders() {
        int count = ((Number) workflow.execute("reminders", Map.of(), "count")).intValue();
    ...
    public ResponseEntity<ApiResponse<Integer>> notificationRetry() {
        int count = ((Number) workflow.execute("notificationRetry", Map.of(), "count")).intValue();
    ...
    public ResponseEntity<ApiResponse<Integer>> ratingUpdate() {
        int count = ((Number) workflow.execute("ratingUpdate", Map.of(), "count")).intValue();
