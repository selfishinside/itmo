...
class Main {
    ...
    static double r; // Изменено на double
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (fcgiInterface.FCGIaccept() >= 0) {
            ...
            try {
                ...
                // Извлекаем данные и парсим r как double
                int x = Integer.parseInt(params.getOrDefault("x", "0"));
                int y = Integer.parseInt(params.getOrDefault("y", "0"));
                r = Double.parseDouble(params.getOrDefault("r", "0")); // Изменено на Double.parseDouble
                
                // Проверка попадания
                boolean insideRectangle = isInsideRectangle(x, y, -r, r, r, r);
    ...
    // Проверка для прямоугольника (верхняя правая часть)
    ...
    // Проверка для треугольника (нижняя левая часть)
    ...
    // Проверка для четверти окружности (верхняя левая часть)
    private static boolean isInsidePath(double x, double y) {
        double centerX = 0;
        double centerY = 0;
        double radius = r / 2;
        
        // Убедимся, что точка находится в верхней левой четверти
